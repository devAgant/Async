...
public class RaidMixin {
    ...
    private boolean addToWave(int wave, RaiderEntity raider, Operation<Boolean> original) {
        synchronized (lock) {
            return original.call(wave, raider);
        }
    }

    @WrapMethod(method = "addToWave(ILnet/minecraft/entity/raid/RaiderEntity;Z)Z")
    private boolean addToWave(int wave, RaiderEntity raider, boolean countHealth, Operation<Boolean> original) {
        synchronized (lock) {
            return original.call(wave, raider, countHealth);
        }
    }
